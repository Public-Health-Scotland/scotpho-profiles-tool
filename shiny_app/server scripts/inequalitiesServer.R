...
#     #trend explanation
#     p("The values in the chart are known as the ", tags$b("'Relative Index of Inequality (RII)'"), "for each year this is calculated using a linear regression model of the social variable (in this case the SIMD quintiles) and the selected indicator measure (e.g. rate of hospitalisations/deaths/etc)."),br(),
...
#     p("There are multiple different ways that health inequality can be measured. Understanding inequality has often requires looking at more than one measure. 
...
#       which quantifies the difference between the most disadvantaged group and the overall average value. 
...
#     p("The ", tags$b("'Potential for improvement'"), " option includes charts which indicate the potential for improvement for a given indicator.  
...
#              class="externallink"),
#       " to subdivide geographic areas into 5 groups (also known as quintiles) experiencing different degrees of relative deprivation. In this tool those living in quintile 1 tend to experience the most relative deprivation and those living in quintile 5 the least.
