...
tabPanel("Rank", icon = icon("signal"), value = "rank",
         wellPanel(#Filter options
           column(width = 4,
                  ...
                  div(title="As there are too many hscp localities or IZs to show in the rank chart a selection must be made to limit localities or IZs to only those within a parent area",
                  conditionalPanel( #Conditional panel for extra dropdown for localities & IZ
                    condition = "input.geotype_rank == 'HSC locality' | input.geotype_rank == 'Intermediate zone' ",
                    selectInput("loc_iz_rank", shiny::HTML("<p><span style='font-weight: 400'>Step 2b. Select a region for localities or intermediate zones</span> </p>"),
                              choices = partnership_name)))
           ),
           column(width = 3,
